...
std::optional<int> parse_int_field(std::string_view obj, std::string_view key) {
    ...
    if (pos >= obj.size() || obj[pos] < '0' || obj[pos] > '9') return std::nullopt;
...
std::vector<std::string_view> split_task_objects(std::string_view json) {
...
void JsonFileStorage::save(const std::vector<std::unique_ptr<Task>>& tasks) {
...
std::vector<std::unique_ptr<Task>> JsonFileStorage::load() {
    ...
    buffer << in.rdbuf();
    if (!in.good() && !in.eof()) {
        throw StorageError("Failed while reading storage file: " + filePath_);
    }
